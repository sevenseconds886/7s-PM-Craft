...
2. **用户提供参考图**（截图/设计稿）→ 从参考图提取设计基因，确认后使用
3. **项目根目录 `design-systems/` 目录下 `.json` 文件** → 扫描并列出：
   - 多个文件 → 主动询问用户选择哪个
   - 单个文件 → 主动询问是否使用该规范，或提供参考图
4. **项目根目录** `.design-system.json` 单文件 → 询问是否使用（向后兼容）
5. **PM-Craft Settings** → `GET http://localhost:3456/api/settings` 读取
6. **默认陶土色系** → 内置于本 Skill（见下方），仅当以上都没有时才回退
...
情况 A — design-systems/ 下有多个文件：
  检测到以下设计规范，请选择：
  1. 小煌柜投取餐 — 品牌橙 #FF6E1B + Zapier 暖色体系
  2. zapier — 饱和橙 #ff4f00 + 奶油暖色
  3. 提供参考图（截图/设计稿），从中提取设计基因
  4. 提供自定义设计系统
  5. 使用 PM-Craft 默认陶土色系

情况 B — design-systems/ 下只有 1 个文件：
  检测到设计规范：小煌柜投取餐（品牌橙 #FF6E1B + 暖色体系）
  是否使用该规范？或提供参考图 / 自定义设计系统 / 陶土色系？

情况 C — 无 design-systems/ 目录，有 .design-system.json：
  检测到项目 .design-system.json，是否使用？或提供参考图 / 陶土色系？

情况 D — 都没有：
  未检测到设计规范文件。
  请提供参考图（截图/设计稿），或使用 PM-Craft 默认陶土色系？
```

**关键规则**：
- 有文件时**不自动使用**，必须询问用户确认
- 单个文件也要询问，给用户提供参考图的机会
- 只有用户确认使用文件、或用户没有任何文件且不提供参考图时，才回退到陶土色系
- 用户提供参考图时，从图中提取配色/字体/圆角等设计基因，生成临时 designSystem 对象
